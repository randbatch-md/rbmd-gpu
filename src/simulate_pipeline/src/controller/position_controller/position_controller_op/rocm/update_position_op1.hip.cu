...
__global__ void UpdatePositionFlagvl(
  ...
  if (tid < num_atoms) {
    ...
    sum_vx +=  cont2 * fx[tid] / mass[typei] * dt * fmt2v;
    sum_vy +=  cont2 * fy[tid] / mass[typei] * dt * fmt2v;
    sum_vz +=  cont2 * fz[tid] / mass[typei] * dt * fmt2v;
...
void UpdatePositionFlagOpvl<device::DEVICE_GPU>::operator()(
   const rbmd::Id num_atoms, const rbmd::Real fmt2v, const rbmd::Real par_a,const rbmd::Real par_b,const rbmd::Real dt, const rbmd::Id test_current_step, Box  box  ,const rbmd::Id* atoms_type,
   const rbmd::Real* fx, const rbmd::Real* fy, const rbmd::Real* fz,const rbmd::Real* mass,
   rbmd::Real* vx, rbmd::Real* vy, rbmd::Real* vz,
   rbmd::Real* px, rbmd::Real* py, rbmd::Real* pz,
   rbmd::Id* flag_px,rbmd::Id* flag_py, rbmd::Id* flag_pz) {
  ...
  CHECK_KERNEL(UpdatePositionFlagvl<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
      num_atoms,fmt2v,par_a,par_b, dt, test_current_step, box, atoms_type, fx, fy, fz, mass, vx, vy, vz, px, py, pz, flag_px, flag_py, flag_pz));
}
