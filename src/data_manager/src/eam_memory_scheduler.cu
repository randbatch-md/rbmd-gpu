...
bool EAMMemoryScheduler::asyncMemoryH2D() {
  ...
  /// frho
  thrust::copy(fd->_h_frho, fd->_h_frho + fd->_num_frho,
               _device_data->_d_frho.begin());

  /// rhor
  thrust::copy(fd->_h_rhor, fd->_h_rhor + fd->_num_rhor,
               _device_data->_d_rhor.begin());

  /// z2r
  thrust::copy(fd->_h_z2r, fd->_h_z2r + fd->_num_z2r,
               _device_data->_d_z2r.begin());
