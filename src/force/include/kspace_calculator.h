...
private:
  ...
  //RBSOG - New methods and members for RBSOG
  void ComputeRBSOG();
  void RBSOGInit();
  void RBSOGSetup();
  void RBSOGSampleKSpace();

  //RBSOG Host-side helper functions (ported from rbsog_intel.cpp)
  ...
  rbmd::Real randn_box_muller( rbmd::Real Mean,  rbmd::Real SquareMargin); // Uses _rbsog_rng
  ...
  thrust::host_vector<Int3>         _h_rbsog_K_Sample_int; // Store (mx,my,mz)
  thrust::host_vector<rbmd::Id>         _h_rbsog_K_Sample_x; // Store (mx,my,mz)
  thrust::host_vector<rbmd::Id>         _h_rbsog_K_Sample_y; // Store (mx,my,mz)
  thrust::host_vector<rbmd::Id>         _h_rbsog_K_Sample_z; // Store (mx,my,mz)
  ...
  thrust::device_vector<rbmd::Id>   _d_rbsog_idx_npt_all; // Gather map
