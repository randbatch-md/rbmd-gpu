...
__global__ void ComputeLJForce(
    ...
    rbmd::Real* force_y, rbmd::Real* force_z, rbmd::Real* evdwl,
...
__device__ rbmd::Real CoulForce(rbmd::Real cut_off, rbmd::Real alpha,
...
void LJForceOp<device::DEVICE_GPU>::operator()(
  ...
  CHECK_KERNEL(ComputeLJForce<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
      ...
      end_id, id_verletlist, px, py, pz, force_x, force_y, force_z, evdwl,
      total_evdwl));
}
