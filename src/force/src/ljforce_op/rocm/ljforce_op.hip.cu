...
__device__ void lj126(rbmd::Real cut_off, rbmd::Real px12, rbmd::Real py12,
                      ...
                      rbmd::Real& f_ij, rbmd::Real& e_ij) {
  const rbmd::Real small_value = 0.0001;
  const rbmd::Real dis_2 = POW(px12, 2) + POW(py12, 2) + POW(pz12, 2);
  const rbmd::Real cut_off_2 = POW(cut_off, 2);

  if (dis_2 < cut_off_2 && dis_2 > small_value) {
    rbmd::Real sigmaij_6 = POW(sigma_ij, 6);
...
__device__ void lj126_rs(rbmd::Real rs, rbmd::Real px12, rbmd::Real py12,
  ...
  if (dis_2 < rs_2 && dis_2 > small_value) {
    rbmd::Real sigmaij_6 =
        sigma_ij * sigma_ij * sigma_ij * sigma_ij * sigma_ij * sigma_ij;
    rbmd::Real dis_6 = dis_2 * dis_2 * dis_2;
    rbmd::Real sigmaij_dis_6 = sigmaij_6 / dis_6;
...
// CoulForce
__device__ void CoulForce(rbmd::Real cut_off, rbmd::Real alpha,
                          rbmd::Real charge_pi, rbmd::Real charge_pj,
                          rbmd::Real px12, rbmd::Real py12, rbmd::Real pz12,
...
// EwaldForce
__device__ void EwaldForce(Box* box, const rbmd::Real alpha, Real3 M,
                           ...
                           rbmd::Real& ewald_force_x, rbmd::Real& ewald_force_y,
...
__global__ void ComputeLJForceViral(
    ...
    const rbmd::Id* end_id, const rbmd::Id* id_verletlist, const rbmd::Real* px,
    const rbmd::Real* py, const rbmd::Real* pz, rbmd::Real* force_x,
    rbmd::Real* force_y, rbmd::Real* force_z, rbmd::Real* virial_xx,
    rbmd::Real* virial_yy, rbmd::Real* virial_zz, rbmd::Real* virial_xy,
    rbmd::Real* virial_xz, rbmd::Real* virial_yz, rbmd::Real* total_evdwl) {
  rbmd::Real sum_fx = 0;
  rbmd::Real sum_fy = 0;
  rbmd::Real sum_fz = 0;
...
__global__ void ComputeLJRBLForce(
    ...
    const rbmd::Id* molecular_type, const rbmd::Real* sigma,
    const rbmd::Real* eps, const rbmd::Id* start_id, const rbmd::Id* end_id,
    const rbmd::Id* id_verletlist, const rbmd::Id* id_random_neighbor,
    const rbmd::Id* random_neighbor_num, const rbmd::Real* px,
    const rbmd::Real* py, const rbmd::Real* pz, rbmd::Real* force_x,
    rbmd::Real* force_y, rbmd::Real* force_z) {
  rbmd::Real sum_fx = 0;
  rbmd::Real sum_fy = 0;
...
// RBL: Fix LJForce
__global__ void FixLJRBLForce(const rbmd::Id num_atoms,
                              const rbmd::Real corr_value_x,
                              const rbmd::Real corr_value_y,
                              const rbmd::Real corr_value_z,
                              rbmd::Real* force_x, rbmd::Real* force_y,
                              rbmd::Real* force_z) {
  unsigned int tid1 = blockIdx.x * blockDim.x + threadIdx.x;
  if (tid1 < num_atoms) {
...
__global__ void ComputeCoulForce(
    Box* box, const rbmd::Real cut_off, const rbmd::Id num_atoms,
    const rbmd::Real alpha, const rbmd::Id* atoms_type,
    const rbmd::Id* molecular_type, const rbmd::Id* start_id,
    const rbmd::Id* end_id, const rbmd::Id* id_verletlist,
    const rbmd::Real* charge, const rbmd::Real* px, const rbmd::Real* py,
    const rbmd::Real* pz, rbmd::Real* force_x, rbmd::Real* force_y,
    rbmd::Real* force_z) {
  rbmd::Real sum_fx = 0;
  rbmd::Real sum_fy = 0;
  rbmd::Real sum_fz = 0;
  ...
  if (tid1 < num_atoms) {
    ...
    for (int j = start_id[tid1]; j < end_id[tid1]; ++j) {
      ...
      rbmd::Real coul_force;
      CoulForce(cut_off, alpha, charge_i, charge_j, px12, py12, pz12,
                coul_force);
...
__global__ void ComputeChargeStructureFactorComponent(
    const rbmd::Id num_atoms, Real3 K, const rbmd::Real* px,
...
__global__ void ComputeEwaldForce(
    ...
    const rbmd::Real alpha, const rbmd::Real* real_array,
      ...
      for (rbmd::Id j = -Kmax; j <= Kmax; ++j) {
        for (rbmd::Id k = -Kmax; k <= Kmax; ++k) {
          if (i != 0 || j != 0 || k != 0) {
            ...
            EwaldForce(box, alpha, M, rhok_real_i, rhok_imag_i, charge_i, p_x,
                       p_y, p_z, force_x, force_y, force_z);
...
void LJForceOp<device::DEVICE_GPU>::operator()(
  ...
  CHECK_KERNEL(ComputeLJForce<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
      ...
      end_id, id_verletlist, px, py, pz, force_x, force_y, force_z,
      total_evdwl));
}
...
void LJForceVirialOp<device::DEVICE_GPU>::operator()(
  ...
  CHECK_KERNEL(ComputeLJForceViral<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
      box, cut_off, num_atoms, atoms_type, molecular_type, sigma, eps, start_id,
      end_id, id_verletlist, px, py, pz, force_x, force_y, force_z, virial_xx,
      virial_yy, virial_zz, virial_xy, virial_xz, virial_yz, total_evdwl));
}
...
void LJRBLForceOp<device::DEVICE_GPU>::operator()(
  ...
  CHECK_KERNEL(ComputeLJRBLForce<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
      box, rs, rc, num_atoms, neighbor_sample_num, pice_num, atoms_type,
      molecular_type, sigma, eps, start_id, end_id, id_verletlist,
      id_random_neighbor, random_neighbor_num, px, py, pz, force_x, force_y,
      force_z));
}
...
void FixLJRBLForceOp<device::DEVICE_GPU>::operator()(
    ...
    rbmd::Real* force_x, rbmd::Real* force_y, rbmd::Real* force_z) {
  unsigned int blocks_per_grid = (num_atoms + BLOCK_SIZE - 1) / BLOCK_SIZE;

  CHECK_KERNEL(FixLJRBLForce<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
      num_atoms, corr_value_x, corr_value_y, corr_value_z, force_x, force_y,
      force_z));
}
...
void LJEnergyOp<device::DEVICE_GPU>::operator()(
  ...
  CHECK_KERNEL(ComputeLJEnergy<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
      box, cut_off, num_atoms, atoms_type, molecular_type, sigma, eps, start_id,
      end_id, id_verletlist, px, py, pz, total_evdwl));
}
...
void CoulForceOp<device::DEVICE_GPU>::operator()(
  ...
  CHECK_KERNEL(ComputeCoulForce<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
      box, cut_off, num_atoms, alpha, atoms_type, molecular_type, start_id,
      end_id, id_verletlist, charge, px, py, pz, force_x, force_y, force_z));
}
...
void ComputeChargeStructureFactorComponentOp<device::DEVICE_GPU>::operator()(
  ...
  CHECK_KERNEL(ComputeChargeStructureFactorComponent<<<blocks_per_grid,
                                                       BLOCK_SIZE, 0, 0>>>(
...
void ComputeEwaldForceOp<device::DEVICE_GPU>::operator()(
  ...
  CHECK_KERNEL(ComputeEwaldForce<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
      box, num_atoms, Kmax, alpha, real_array, imag_array, charge, px, py, pz,
      ewald_force_x, ewald_force_y, ewald_force_z));
}
