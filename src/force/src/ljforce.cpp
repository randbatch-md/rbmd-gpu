...
LJForce::~LJForce() { CHECK_RUNTIME(FREE(_d_total_evdwl)); }
...
void LJForce::Execute() {
  ...
  if (neighbor_type == "RBL")  // RBL
  {
    ...
    lj_rbl_force_op(
        _device_data->_d_box, r_core, cut_off, _num_atoms, neighbor_sample_num,
...
void LJForce::ComputeChargeStructureFactorEwald(Box* box, rbmd::Id num_atoms,
                                                rbmd::Id Kmax,
  ...
  rbmd::Id total_elements =
      (2 * Kmax + 1) * (2 * Kmax + 1) * (2 * Kmax + 1) - 1;
    ...
    for (rbmd::Id j = -Kmax; j <= Kmax; j++) {
      for (rbmd::Id k = -Kmax; k <= Kmax; k++) {
        if (!(i == 0 && j == 0 && k == 0)) {
          ...
          op::ComputeChargeStructureFactorComponentOp<device::DEVICE_GPU>
              charge_structure_factor_op;
          charge_structure_factor_op(
              num_atoms, K,
              thrust::raw_pointer_cast(_device_data->_d_px.data()),
              thrust::raw_pointer_cast(_device_data->_d_py.data()),
          ...
          rbmd::Real value_Re =
              thrust::reduce(density_real.begin(), density_real.end(), 0.0f,
                             thrust::plus<rbmd::Real>());
          rbmd::Real value_Im =
              thrust::reduce(density_imag.begin(), density_imag.end(), 0.0f,
                             thrust::plus<rbmd::Real>());
...
void LJForce::ComputeEwladForce() {
  ...
  ewlad_force_op(_device_data->_d_box, _num_atoms, Kmax, alpha, value_Re_array,
                 value_Im_array,
                 thrust::raw_pointer_cast(_device_data->_d_charge.data()),
                 thrust::raw_pointer_cast(_device_data->_d_px.data()),
                 thrust::raw_pointer_cast(_device_data->_d_py.data()),
