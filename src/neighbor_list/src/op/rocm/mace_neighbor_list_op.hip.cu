...
#include "model/box.h"
#include "src/op/mace_neighbor_list_op.h"

namespace op {
__global__ void ComputeMACENeighbors(rbmd::Id* per_dimension_cells,
...
__global__ void ComputeMACENeighborsWithoutPBC(rbmd::Id* neighbor_cell,
...
__global__ void EstimateMACENeighborList(
...
__global__ void GenerateMaceNeighborList(
    rbmd::Id* __restrict__ per_atom_cell_id,
    rbmd::Id* __restrict__ in_atom_list_start_index,
    rbmd::Id* __restrict__ in_atom_list_end_index, rbmd::Real cutoff_2,
    rbmd::Id total_atom_num, rbmd::Real* __restrict__ px,
    ...
    rbmd::Id* __restrict__ neighbors, Box  d_box,
    ...
    for (int i = 0; i < neighbor_cell_num; ++i) {
      ...
      for (rbmd::Id base_idx = start; base_idx < end; base_idx += warpSize) {
        ...
        if (is_neighbor) {
          ...
          neighbors_atoms[neighbor_num + offset] = atom_idx;
          double shiftxd = px[neighbor_atom_idx]-px[atom_idx];
          double shiftyd = py[neighbor_atom_idx]-py[atom_idx];
          double shiftzd = pz[neighbor_atom_idx]-pz[atom_idx];
          double shiftxs = std::round(d_box._length_inv[0]*shiftxd + d_box._length_inv[5]*shiftyd + d_box._length_inv[4]*shiftzd);
          double shiftys = std::round(d_box._length_inv[1]*shiftyd + d_box._length_inv[3]*shiftzd);
          double shiftzs = std::round(d_box._length_inv[2]*shiftzd);
          unit_shiftx[neighbor_num + offset] = shiftxs;
          unit_shifty[neighbor_num + offset] = shiftys;
          unit_shiftz[neighbor_num + offset] = shiftzs;
          shiftx[neighbor_num + offset] = d_box._length[0]*shiftxs + d_box._length[5]*shiftys + d_box._length[4]*shiftzs;
          shifty[neighbor_num + offset] = d_box._length[1]*shiftys + d_box._length[3]*shiftzs;
          shiftz[neighbor_num + offset] = d_box._length[2]*shiftzs;
        }
        ...
        //mace优化尝试
        //neighbor_num +=_neighbor_list->_d_neighbor_num[];
      }
    }
...
__global__ void GenerateMaceNeighborListNoWarp(
    rbmd::Id* __restrict__ per_atom_cell_id,
    rbmd::Id* __restrict__ in_atom_list_start_index,
    rbmd::Id* __restrict__ in_atom_list_end_index, rbmd::Real cutoff_2,
    rbmd::Id total_atom_num, rbmd::Real* __restrict__ px,
      ...
      for (rbmd::Id neighbor_atom_idx = start; neighbor_atom_idx < end;
        ...
        if (atom_idx != neighbor_atom_idx && neighbor_atom_idx < end) {
          ...
          if (distance < cutoff_2) {
            ...
            //neighbors[neighbor_num ] = neighbor_atom_idx;
            neighbors_atoms[neighbor_num ] = atom_idx;
            double shiftxd = px[neighbor_atom_idx]-px[atom_idx];
            double shiftyd = py[neighbor_atom_idx]-py[atom_idx];
            double shiftzd = pz[neighbor_atom_idx]-pz[atom_idx];
            double shiftxs = std::round(d_box._length_inv[0]*shiftxd + d_box._length_inv[5]*shiftyd + d_box._length_inv[4]*shiftzd);
            double shiftys = std::round(d_box._length_inv[1]*shiftyd + d_box._length_inv[3]*shiftzd);
            double shiftzs = std::round(d_box._length_inv[2]*shiftzd);
            unit_shiftx[neighbor_num ] = shiftxs;
            unit_shifty[neighbor_num] = shiftys;
            unit_shiftz[neighbor_num ] = shiftzs;
            shiftx[neighbor_num ] = d_box._length[0]*shiftxs + d_box._length[5]*shiftys + d_box._length[4]*shiftzs;
            shifty[neighbor_num ] = d_box._length[1]*shiftys + d_box._length[3]*shiftzs;
            shiftz[neighbor_num ] = d_box._length[2]*shiftzs;
            ++neighbor_num;
          }
        }
           }
...
void ComputeMACENeighborsOp<device::DEVICE_GPU>::operator()(
    rbmd::Id* per_dimension_cells, rbmd::Id* neighbor_cell,
    rbmd::Id neighbor_num, rbmd::Id total_cell,
    rbmd::Id cell_count_within_cutoff) {
  unsigned int blocks_per_grid = (total_cell + BLOCK_SIZE - 1) / BLOCK_SIZE;
  CHECK_KERNEL(ComputeMACENeighbors<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
      per_dimension_cells, neighbor_cell, neighbor_num, total_cell,
      cell_count_within_cutoff));
}

void ComputeMACENeighborsWithoutPBCOp<device::DEVICE_GPU>::operator()(
    rbmd::Id* neighbor_cell, rbmd::Id neighbor_num, rbmd::Id total_cell) {
  unsigned int blocks_per_grid = (total_cell + BLOCK_SIZE - 1) / BLOCK_SIZE;
  CHECK_KERNEL(
      ComputeMACENeighborsWithoutPBC<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
          neighbor_cell, neighbor_num, total_cell));
}

void EstimateMACENeighborListOp<device::DEVICE_GPU>::operator()(
  ...
  CHECK_KERNEL(
      EstimateMACENeighborList<<<blocks_per_grid, BLOCK_SIZE,
...
void GenerateMACENeighborListOp<device::DEVICE_GPU>::operator()(
rbmd::Id* __restrict__ per_atom_cell_id,
rbmd::Id* __restrict__ in_atom_list_start_index,
rbmd::Id* __restrict__ in_atom_list_end_index, rbmd::Real cutoff_2,
rbmd::Id total_atom_num, rbmd::Real* __restrict__ px,
...
rbmd::Id* __restrict__ max_neighbor_num,
rbmd::Id* __restrict__ neighbor_start, rbmd::Id* __restrict__ neighbor_end,
rbmd::Id* __restrict__ neighbors, Box _box,
rbmd::Id* __restrict__ neighbors_atoms, rbmd::Real* __restrict__  unit_shiftx,
rbmd::Real* __restrict__  unit_shifty,  rbmd::Real* __restrict__  unit_shiftz,
rbmd::Real* __restrict__  shiftx,	rbmd::Real* __restrict__  shifty,
rbmd::Real* __restrict__  shiftz,
...
rbmd::Id neighbor_cell_num) {
  if (total_atom_num < 10000) {  // TODO 精确测试数值
    ...
    unsigned int blocks_per_grid =
        (total_atom_num + warps_per_block - 1) / warps_per_block;
    CHECK_KERNEL(
        GenerateMaceNeighborList<<<blocks_per_grid, BLOCK_SIZE,
                                   sizeof(hipcub::WarpScan<int>::TempStorage) *
                                       (BLOCK_SIZE / WARP_SIZE),
                                   0>>>(
            per_atom_cell_id, in_atom_list_start_index, in_atom_list_end_index,
            cutoff_2, total_atom_num, px, py, pz, max_neighbor_num,
            neighbor_start, neighbor_end, neighbors, _box,
            neighbors_atoms,unit_shiftx,unit_shifty,unit_shiftz,shiftx,shifty,shiftz,
            should_realloc,
    ...
    CHECK_KERNEL(
        GenerateMaceNeighborListNoWarp<<<blocks_per_grid, BLOCK_SIZE, 0, 0>>>(
           per_atom_cell_id, in_atom_list_start_index, in_atom_list_end_index,
           cutoff_2, total_atom_num,px, py, pz, max_neighbor_num,
           neighbor_start, neighbor_end, neighbors, _box,
           neighbors_atoms,unit_shiftx,unit_shifty,unit_shiftz,shiftx,shifty,shiftz,
