...
// TODO ！ 考虑可能是 hipcub::WarpScan<int>(temp_storage[threadIdx.x / warpSize]).ExclusiveSum(is_neighbor, offset);导致的
    ...
    for (int i = 0; i < neighbor_cell_num; ++i) {
      ...
      for (rbmd::Id neighbor_atom_idx = start; neighbor_atom_idx < end;
        ...
        if (valid_neighbor) {
          distance =
              CaculateDistance(d_box, px[atom_idx], py[atom_idx], pz[atom_idx],
                               px[neighbor_atom_idx],
                               py[neighbor_atom_idx], pz[neighbor_atom_idx]);
          if (distance < cutoff_2) {
