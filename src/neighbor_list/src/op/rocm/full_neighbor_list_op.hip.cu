    ...
    for (int i = 0; i < neighbor_cell_num; ++i) {
      ...
      for (rbmd::Id base_idx = start; base_idx < end; base_idx += warpSize) {
        ...
        if (is_neighbor) {
          ...
          double shiftxd = &px[atom_idx] - &px[neighbor_atom_idx];
          double shiftyd = &py[atom_idx] - &py[neighbor_atom_idx];
          double shiftzd = &pz[atom_idx] - &pz[neighbor_atom_idx];
  ...
  if (total_atom_num < 10000) {  // TODO 精确测试数值
    ...
    CHECK_KERNEL(
        GenerateMaceNeighborList<<<blocks_per_grid, BLOCK_SIZE,
            ...
            per_atom_cell_id, in_atom_list_start_index, in_atom_list_end_index,
            cutoff_2, total_atom_num, px, py, pz, max_neighbor_num,
            neighbor_start, neighbor_end, neighbors, d_box, neighbors_atoms,unit_shiftx,unit_shifty,unit_shiftz,shiftx,shifty,shiftz,should_realloc,
            neighbor_cell, neighbor_cell_num));
  } else {
    unsigned int blocks_per_grid =
        (total_atom_num + BLOCK_SIZE - 1) / BLOCK_SIZE;
