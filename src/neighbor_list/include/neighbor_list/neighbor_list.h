...
class NeighborList {
 public:
  ...
  thrust::device_vector<rbmd::Id>
      _d_max_neighbor_num{};  // 每个原子的最大邻居容量
  rbmd::Id _h_total_max_neighbor_num = 0;
  thrust::device_vector<rbmd::Id>
      _d_neighbors{};  // 每个原子的邻居列表  when use rbl is: s_neighbor
  thrust::device_vector<rbmd::Id>
      _start_idx{};  // 每个原子的邻居在_d_neighbors的起始位置
  thrust::device_vector<rbmd::Id>
      _end_idx{};  // 每个原子的邻居在_d_neighbors的结束位置
  thrust::device_vector<rbmd::Id>
      _d_random_neighbor{};  // just for rbl  cs neighbor  开始：atom_id  *
                             // neighbor_sample_num
  thrust::device_vector<rbmd::Id>
